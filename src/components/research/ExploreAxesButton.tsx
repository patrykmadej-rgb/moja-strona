...
export default function ExploreAxesButton({ label }: { label: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="mt-8 inline-flex items-center gap-2 bg-[#4A1D6E] px-7 py-3 text-sm font-normal text-white transition-colors hover:bg-[#4A2073] dark:hover:bg-[#7B4DB8]"
    >
      {label} ↓
    </button>
  );
